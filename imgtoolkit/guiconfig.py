r"""Tiny JSON-backed config for the Image Toolkit GUI.

Stores just two things and never raises: the chosen theme ("light"/"dark") and a
short list of recent image files.  On Windows the file lives at
``%LOCALAPPDATA%\ImageToolkit\config.json``; elsewhere it falls back to
``~/.imagetoolkit/config.json``.  Every function is defensive -- a corrupt or
...
def config_dir():
    ...
    ``%LOCALAPPDATA%\ImageToolkit`` on Windows, ``~/.imagetoolkit`` otherwise.
    """
